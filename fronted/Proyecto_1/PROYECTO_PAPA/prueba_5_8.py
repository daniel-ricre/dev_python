...
sg.theme('DarkBlack1')  # Base pura
sg.set_options(font=('Segoe UI', 10), dpi_awareness=True)

# --- Constantes de Diseño (Inspirado en Linear) ---
COLORS = {
    'bg': '#0A0A0A',
    'surface': '#18181B',
    'border': '#27272A',
...
# --- Funciones de Excel (Refactorizadas y Optimizadas) ---
        ...
        for day in week:
            if day == 0:
                ...
                btn = sg.Button(str(day), size=(2, 1), key=f'-CAL-{day}',
                                button_color=(txt, bg), border_width=0, font=('Segoe UI', 9, 'bold'),
                                pad=(1, 2))
                row.append(btn)
        layout.append(row)
...
# --- Layout Principal (Optimizado con Columnas Fijas) ---
...
# --- Ensamblaje de Ventana ---
layout = [
    [sidebar_col, sg.VSeparator(color=COLORS['border']), data_col, sg.VSeparator(
        color=COLORS['border']), form_col]
]

window = sg.Window('Inventory Flow', layout, size=(1400, 750), margins=(0, 0),
                   background_color=COLORS['bg'], resizable=True, finalize=True)

# --- Función Mágica de Refresco UI Completa (Corrige el Crash) ---


def refresh_ui_components():
    # 1. Actualizar Botones de Menú
    ...
    # 2. Actualizar Títulos y Colores Contextuales
    ...
    # 3. Forzar refresco del Calendario (Recarga el layout)
...
# --- Bucle de Eventos (Ultra Estable) ---
while True:
    ...
    # === Manejo de Categorías (CORREGIDO: Ya no cierra) ===
    if event == '-CAT-VINO-':
        categoria_activa = 'Vino'
        refresh_ui_components()
    ...
    if event == '-GUARDAR-':
        ...
        if not nombre or not cantidad:
            window['-MSG-'].update('❌ Error: Campos vacíos',
