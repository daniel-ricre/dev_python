...
def inicializar_excel():
    ...
    if not os.path.exists(ARCHIVO_EXCEL):
        ...
        for categoria, etiquetas in ETIQUETAS_CATEGORIA.items():
            ...
            # Encabezados de la tabla (AHORA CON FECHA Y HORA SEPARADOS)
            ...
            if categoria != 'Dulces':
                ws.merge_cells(f'A{fila_actual}:D{fila_actual}')  # 4 columnas
...
def guardar_en_excel(categoria, nombre, cantidad, fecha=None, hora=None):
    ...
    try:
        ...
        for col in range(1, 5):  # Ahora son 4 columnas
...
layout = [
    ...
    # === CONTENIDO PRINCIPAL ===
    [
        # COLUMNA IZQUIERDA: Tabla de registros (AHORA CON 4 COLUMNAS)
        sg.Column([
    ...
    # === BARRA DE ESTADO ===
    [sg.HorizontalSeparator()],
    [sg.Text('💡 Tip: La fecha y hora se guardan por separado para mejor organización',
