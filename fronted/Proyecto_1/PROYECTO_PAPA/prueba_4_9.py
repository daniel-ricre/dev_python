...
def inicializar_excel():
    ...
    if not os.path.exists(ARCHIVO_EXCEL):
        ...
        for categoria, etiquetas in ETIQUETAS_CATEGORIA.items():
            ...
            # Agregar una fila vacía para datos
            fila_actual += 1

            # Línea divisoria gruesa entre tablas (sin merge cells problemático)
            if categoria != 'Dulces':
                # Usar borde en una fila normal en lugar de merged cell
                for col in range(1, 5):
                    celda = ws.cell(row=fila_actual, column=col)
                    celda.border = Border(bottom=Side(
                        style='thick', color='000000'))
                    celda.value = ""  # Celda vacía
...
def guardar_en_excel(categoria, nombre, cantidad, fecha=None, hora=None):
    ...
    try:
        ...
        # Encontrar la primera fila vacía en esa tabla (EVITANDO CELDAS COMBINADAS)
        while True:
        ...
        # Insertar el nuevo registro (4 columnas)
...
layout = [
    ...
    # === BARRA DE ESTADO ===
    [sg.HorizontalSeparator()],
    [sg.Text('💡 Tip: Puedes guardar todos los productos que quieras sin límite',
