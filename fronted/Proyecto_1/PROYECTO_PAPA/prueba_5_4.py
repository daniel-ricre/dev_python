...
COLORS = {
    ...
    'dark': '#1E293B',         # Gris pizarra oscuro
    ...
    if not os.path.exists(ARCHIVO_EXCEL):
        ...
        for categoria, nombre_hoja in HOJAS_EXCEL.items():
            ...
            celda_titulo.fill = PatternFill(
                start_color=CATEGORIAS[categoria]['color'].replace('#', ''),
                end_color=CATEGORIAS[categoria]['color'].replace('#', ''),
                fill_type='solid'
            )
            ...
            for col, encabezado in enumerate(encabezados, start=1):
                ...
                celda.fill = PatternFill(
                    start_color=CATEGORIAS[categoria]['gradient'][1].replace(
                        '#', ''),
                    end_color=CATEGORIAS[categoria]['gradient'][1].replace(
                        '#', ''),
                    fill_type='solid'
                )
...
def mostrar_calendario(fecha_actual, fecha_seleccionada_anterior=None):
    ...
    for semana in cal:
        ...
        for dia in semana:
            if dia == 0:
                ...
                key = f'-DIA_{dia}-'
                btn = sg.Button(str(dia), size=(4, 2), key=key,
                                button_color=color, font=(
                                    'Segoe UI', 9, 'bold'),
                                border_width=1)
                fila.append(btn)
                botones[key] = btn
    ...
    layout_cal.extend([
        [sg.Text('')],
        [sg.Button('◀', size=(4, 1), key='-MES_ANT-', button_color=(COLORS['text'], COLORS['light']), border_width=1),
         sg.Button('Hoy', size=(6, 1), key='-HOY-',
                   button_color=('white', COLORS['primary']), border_width=1),
         sg.Button('▶', size=(4, 1), key='-MES_SIG-', button_color=(COLORS['text'], COLORS['light']), border_width=1)],
        [sg.Text('')],
        [sg.Button('Cancelar', size=(10, 1), button_color=(COLORS['text'], COLORS['light']), border_width=1),
         sg.Button('Seleccionar', size=(10, 1), button_color=('white', COLORS['success']), border_width=1, key='-SEL-')]
    ])
...
# --- Función para crear un botón de categoría estilizado ---


def cat_button(text, key, color):
    return sg.Button(text, size=(14, 2), key=key,
...
# Tarjeta de tabla (panel izquierdo)
tabla_card = [
...
# Tarjeta de formulario (panel derecho)
...
def crear_formulario():
    ...
    return [
        ...
        [sg.Input(key='-CANTIDAD-', size=(28, 1), font=('Segoe UI', 11),
                  border_width=1, relief=sg.RELIEF_SOLID)],
        [sg.Text('')],
        [sg.Text('📅 Fecha', font=('Segoe UI', 10, 'bold'),
                 text_color=COLORS['text'])],
        [sg.Input(key='-FECHA_REGISTRO-', size=(28, 1),
                  default_text=fecha_actual.strftime("%Y-%m-%d"),
                  font=('Segoe UI', 10), border_width=1, relief=sg.RELIEF_SOLID)],
        [sg.Text('')],
        [sg.Text('🕐 Hora', font=('Segoe UI', 10, 'bold'),
                 text_color=COLORS['text'])],
        [sg.Input(key='-HORA_REGISTRO-', size=(28, 1),
                  default_text=hora_actual,
                  font=('Segoe UI', 10), border_width=1, relief=sg.RELIEF_SOLID)],
        [sg.Text('')],
        [sg.Button('💾  GUARDAR', size=(20, 2),
                   button_color=('white', COLORS['primary']),
                   font=('Segoe UI', 11, 'bold'),
                   key='-GUARDAR-', border_width=0)],
        [sg.Text('')],
...
# Layout principal con estructura de tarjetas
layout = [
    header,
    [sg.Text('')],
    [sg.Frame('', categoria_card, relief=sg.RELIEF_FLAT, border_width=0,
              element_justification='center', expand_x=True)],
    [sg.Text('')],
    [sg.Frame('', fecha_card, relief=sg.RELIEF_FLAT, border_width=0,
              element_justification='center', expand_x=True)],
    [sg.Text('')],
    [
        sg.Frame('', tabla_card, relief=sg.RELIEF_FLAT, border_width=1,
                 border_color=COLORS['border'], expand_x=True, expand_y=True),
        sg.Column([[sg.VSeparator(color=COLORS['border'])]]),
        sg.Frame('', crear_formulario(), relief=sg.RELIEF_FLAT, border_width=1,
                 border_color=COLORS['border'], key='-FRAME_FORM-')
    ],
    [sg.Text('')],
...
window = sg.Window('🏪 Inventario Pro', layout, size=(1100, 700),
                   ...
                   margins=(15, 15), element_padding=(5, 5))
...
def cambiar_categoria(nueva_cat):
    ...
    window['-FRAME_FORM-'].Widget.children.clear()
    for fila in crear_formulario():
        window.extend_layout(window['-FRAME_FORM-'], [fila])
